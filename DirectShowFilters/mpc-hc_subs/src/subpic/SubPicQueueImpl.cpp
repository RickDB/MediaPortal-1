                ...
                if (rtCurrent < rtStop) {
                    ...
                    while (rtCurrent < rtStop) {
                        ...
                        if (bIsAnimated) {
                            ...
                            hr = RenderTo(pStatic, rtCurrent, min(rtCurrent + rtTimePerSubFrame * 3 / 4, rtStopReal), fps, bIsAnimated);
                            ...
                            rtCurrent = min(rtCurrent + rtTimePerSubFrame, rtStopReal);
                        } else {
    ...
    if (pSubPic && pSubPic->GetStart() <= rtNow && rtNow < pSubPic->GetStop()) {
        ...
        if (SUCCEEDED(GetSubPicProvider(&pSubPicProvider)) && pSubPicProvider
            ...
            if (pos) {
                ...
                if (bAnimated) {
                    rtStart = rtNow;
                    rtStop = min(rtNow + m_rtTimePerSubFrame, rtStop);
                } else {
