...
CSubtitle* CRenderedTextSubtitle::GetSubtitle(int entry)
{
    ...
    if (sub->m_relativeTo == STSStyle::VIDEO) {
        ...
        if (marginRect.left < 0) {
            clipRect.left = max(0l, clipRect.left + marginRect.left);
        }
        if (marginRect.top < 0) {
            clipRect.top = max(0l, clipRect.top + marginRect.top);
        }
        if (marginRect.right < 0) {
            clipRect.right = min(m_size.cx, clipRect.right - marginRect.right);
        }
        if (marginRect.bottom < 0) {
            clipRect.bottom = min(m_size.cy, clipRect.bottom - marginRect.bottom);
        }
